...
function PaymentMethodsPanel({ profile }: { profile: Profile | null }) {
    ...
    setEditing((prev) => {
      ...
      if (typeof value === 'string') {
        ...
        if (field === 'securityCode') {
          return { ...prev, securityCode: value.replace(/\D/g, '').slice(0, 4) }
        }
  ...
  const handleEditSubmit = () => {
    ...
    if (editing.type !== 'BANK_ACCOUNT') {
      const trimmedSecurity = editing.securityCode.replace(/\D/g, '')
      if (trimmedSecurity.length) {
        updates.securityCode = trimmedSecurity
      }
    }
